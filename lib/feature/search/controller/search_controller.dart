import 'package:purplestage/feature/search/model/filter_model.dart';
import 'package:purplestage/feature/search/model/provider_suggestion_model.dart';
import 'package:purplestage/feature/search/model/search_service_model.dart';
...
class AllSearchController extends GetxController implements GetxService {
  ...
  ProviderSuggestionModel? _providerSuggestionModel;
  ProviderSuggestionModel? get providerSuggestionModel => _providerSuggestionModel;

  List<SearchSuggestion> _searchSuggestionList = [];
  List<SearchSuggestion> get searchSuggestionList => _searchSuggestionList;

  List<ProviderSuggestion> _providerSuggestionList = [];
  List<ProviderSuggestion> get providerSuggestionList => _providerSuggestionList;
  ...
  List<ProviderData> providerSuggestions = [];
  ...
  Future<void> getSearchSuggestion(String query, {bool shouldUpdate = true,}) async {
    Response response = await searchRepo.getSearchSuggestion(query: query);
    if (response.statusCode == 200) {
  ...
  Future<void> getProviderSuggestion(String query, {bool shouldUpdate = true,}) async {
    Response response = await searchRepo.getProviderSuggestion(query: searchController.text);
    if (response.statusCode == 200) {
      _providerSuggestionModel = ProviderSuggestionModel.fromJson(response.body);

      if(_providerSuggestionModel!=null && _providerSuggestionModel!.providerSuggestionContent != null && _providerSuggestionModel!.providerSuggestionContent!.isNotEmpty){
        _providerSuggestionList = [];
        _providerSuggestionList = _providerSuggestionModel!.providerSuggestionContent! ;
        print(_providerSuggestionList[0].name);
      }
    } else {
      ApiChecker.checkApi(response);
    }
